...
export default function Customers() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Create customer dialog — 4 tabs, fixed bottom save */}
      <Dialog open={createOpen} onOpenChange={(v) => { setCreateOpen(v); if (!v) setCreateError(null); }}>
        <DialogContent className="max-w-2xl h-[85vh] flex flex-col p-0 gap-0">
          <DialogHeader className="px-6 py-4 border-b shrink-0">
            <DialogTitle className="flex items-center gap-2">
              <UserPlus className="h-4 w-4" />{t("Nouveau client", "عميل جديد")}
            </DialogTitle>
          </DialogHeader>
          ...
          <Tabs defaultValue="basic" className="flex-1 flex flex-col overflow-hidden">
            ...
            <TabsContent value="basic" className="flex-1 overflow-y-auto p-6 mt-0 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input id="cn-name" value={createForm.name} onChange={(e) => setCreateForm({ ...createForm, name: e.target.value })} autoFocus data-testid="input-customer-name" className="h-8 text-sm" />
                </div>
                <div>
                  <Label htmlFor="cn-email">{t("Email *", "البريد *")}</Label>
                  <Input id="cn-email" type="email" value={createForm.email} onChange={(e) => setCreateForm({ ...createForm, email: e.target.value })} data-testid="input-customer-email" className="h-8 text-sm" />
                </div>
                <div>
                  <Label htmlFor="cn-phone">{t("Téléphone", "الهاتف")}</Label>
                  <Input id="cn-phone" type="tel" value={createForm.phone} onChange={(e) => setCreateForm({ ...createForm, phone: e.target.value })} placeholder="+213 ..." data-testid="input-customer-phone" className="h-8 text-sm" />
                </div>
                <div>
                  <Label htmlFor="cn-type">{t("Type de contact", "نوع الاتصال")}</Label>
                  <Select value={createForm.contactType} onValueChange={(v) => setCreateForm({ ...createForm, contactType: v })}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select value={createForm.wilaya} onValueChange={(v) => setCreateForm({ ...createForm, wilaya: v === "none" ? "" : v, commune: "" })}>
                ...
                <div>
                  ...
                  {(() => {
                    const wObj = WILAYAS.find((w) => w.nameFr === createForm.wilaya);
                    const comms = wObj ? getCommunesByWilaya(wObj.code) : [];
                    return (
                      <CommuneCombobox
                        communes={comms}
                        value={createForm.commune}
                        onChange={(v) => setCreateForm({ ...createForm, commune: v })}
                        placeholder={t("Choisir...", "اختر...")}
                        searchPlaceholder={t("Rechercher commune...", "بحث عن بلدية...")}
                        emptyText={t("Aucune commune trouvée", "لا توجد نتائج")}
                ...
                <div className="col-span-2">
                  ...
                  <Input id="cn-address" value={createForm.address} onChange={(e) => setCreateForm({ ...createForm, address: e.target.value })} data-testid="input-customer-address" className="h-8 text-sm" />
                </div>
                <div className="col-span-2">
                  <Label htmlFor="cn-gps">{t("GPS (lat,lng)", "GPS (خط,طول)")}</Label>
                  <Input id="cn-gps" value={createForm.gps} onChange={(e) => setCreateForm({ ...createForm, gps: e.target.value })} className="h-8 text-sm font-mono" placeholder="36.737,3.086" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select value={createForm.classificationId} onValueChange={(v) => setCreateForm({ ...createForm, classificationId: v === "none" ? "" : v })}>
                    <SelectTrigger className="h-8 text-sm"><SelectValue placeholder={t("Aucune", "بدون")} /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">{t("Aucune", "بدون")}</SelectItem>
                      {(classifs as CustomerClassification[]).map((c) => (
                ...
                <div>
                  ...
                  <Select value={createForm.priceTierId} onValueChange={(v) => setCreateForm({ ...createForm, priceTierId: v === "none" ? "" : v })}>
                    <SelectTrigger className="h-8 text-sm"><SelectValue placeholder={t("Standard", "قياسي")} /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">{t("Standard", "قياسي")}</SelectItem>
                      {(tiers as PriceTier[]).map((tier) => (
            ...
            <TabsContent value="financial" className="flex-1 overflow-y-auto p-6 mt-0 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input id="cn-accno" value={createForm.accountNumber} onChange={(e) => setCreateForm({ ...createForm, accountNumber: e.target.value })} className="h-8 text-sm font-mono" />
                </div>
                <div>
                  <Label htmlFor="cn-balance">{t(`Solde initial (${currency})`, `الرصيد الابتدائي (${currency})`)}</Label>
                  <Input id="cn-balance" type="number" value={createForm.currentBalance} onChange={(e) => setCreateForm({ ...createForm, currentBalance: e.target.value })} className="h-8 text-sm" />
                </div>
                <div>
                  <Label htmlFor="cn-credit">{t(`Plafond crédit (${currency})`, `سقف الائتمان (${currency})`)}</Label>
                  <Input id="cn-credit" type="number" value={createForm.creditLimit} onChange={(e) => setCreateForm({ ...createForm, creditLimit: e.target.value })} placeholder="0.00" className="h-8 text-sm" />
                </div>
                <div>
                  <Label htmlFor="cn-minalert">{t(`Alerte solde min. (${currency})`, `تنبيه الرصيد الأدنى (${currency})`)}</Label>
                  <Input id="cn-minalert" type="number" value={createForm.minBalanceAlert} onChange={(e) => setCreateForm({ ...createForm, minBalanceAlert: e.target.value })} placeholder="0.00" className="h-8 text-sm" />
                </div>
              </div>
              <div className="flex items-center gap-3 pt-1">
                <Switch checked={createForm.foreignCurrency} onCheckedChange={(v) => setCreateForm({ ...createForm, foreignCurrency: v })} />
            ...
            <TabsContent value="notes" className="flex-1 overflow-y-auto p-6 mt-0 space-y-4">
              ...
              <div>
                ...
                <Input id="cn-pwd" type="text" value={createForm.password} onChange={(e) => setCreateForm({ ...createForm, password: e.target.value })} placeholder={t("Auto-généré si vide", "يُولّد تلقائياً")} data-testid="input-customer-password" className="h-8 text-sm" />
          ...
          <div className="px-6 py-3 border-t bg-background flex items-center gap-3 shrink-0">
            ...
            <Button variant="outline" onClick={() => setCreateOpen(false)}>{t("Annuler", "إلغاء")}</Button>
            <Button onClick={handleCreate} disabled={createCustomer.isPending} className="bg-[#1B3057] hover:bg-[#1B3057]/90" data-testid="button-save-customer">
              {createCustomer.isPending ? "..." : t("Enregistrer", "حفظ")}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
